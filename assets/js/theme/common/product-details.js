...
export default class ProductDetails extends ProductDetailsBase {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            if (Object.keys(productAttributesData).length) {
                if ($changedOption.hasClass("select-hire-designer")) {
                    $('form[data-cart-item-add]', this.$scope).trigger("submit")
                }
            }
        });
    }
    ...
    bindFancyBox() {
        ...
        if ($("#open_fancybox", this.$scope).length == 0) {
            $(".productView-images", this.$scope).length &&
                $(".productView-images", this.$scope).wrap(`<a href="javascript:void(0)" id="openFancybox"></a>`);
            ...
            $(".slider-single", this.$scope).on("swipe", (event, slick, direction) => {
                let c = $(`.main-image-slide.slick-active .main-image`, this.$scope);
                c.find("img").attr({
                    "src": c.attr("href"),
                    "srcset":c.attr("href"),
                    "data-srcset": c.attr("href")
                })
                $(`[data-image-gallery]`, this.$scope).attr("data-zoom-image", c.attr("href"));
            });

            $(".slider-single", this.$scope).on('beforeChange', (event, slick) => {
                let c = $(`.main-image-slide.slick-active .main-image`, this.$scope);
                c.find("img").attr({
                    "src": c.attr("href"),
                    "srcset":c.attr("href"),
