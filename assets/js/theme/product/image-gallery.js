...
export default class ImageGallery {
    ...
    swapMainImage() {
        ...
        try {
            let selector = this.$mainImage.find(".slider-single .slick-active").length > 0 ? '.slick-active': '.productView-img-container a';

            if(this.$mainImage.find(".slider-single .slick-active").length > 0){
                this.easyzoom.data('easyZoom').opts.beforeShow = ()=>{
                    this.easyzoom.data('easyZoom').$flyout.find("img").attr("src",this.currentImage.zoomImageUrl)
                }
            }else{
