...
import {makeSelect2Dropdowns} from "./dh-custom/common";
...
export default class Search extends CatalogPage {
    ...
    onReady() {
        ...
        $("body").on("select2:select",".category-sort-select",(event)=>{
            this.facetedSearch ?           
            this.facetedSearch.onSortBySubmit(event, event.target):
            this.onSortBySubmit(event, event.target);
        });

        $("body").on("select2:select","#articalsort",(event)=>{
            this.facetedSearch.onSortBySubmit(event, event.target);
        });
